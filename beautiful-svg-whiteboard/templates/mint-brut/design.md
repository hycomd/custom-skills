...
description: >
  A candy-bright neo-brutalist design system for standalone SVG whiteboards, drawn from playful mobile-UI mockups. A mint-green canvas carries flat cream cards capped by hot-pink header bands, all bound by heavy 3–4px black borders with softly rounded corners. Type is a clean bold register — no display typeface survives, so weight and scale do the work — and bright pink / green / gold accents pop as solid color-blocks with bold accent chips breaking the grid. FLAT throughout: no shadows, no gradients. Built for friendly explainers, onboarding flows, step sequences, and comparison or system maps that should feel approachable rather than corporate.
...
# Mint Brut — SVG Whiteboard Design System

A design system for building **friendly explanatory diagrams** as editable standalone SVG
